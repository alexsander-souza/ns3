 ...
 * frames from one port may occasionally be forwarded throughout all other
 * ports,
...
public:
   ...
   * \brief These four data describe the OpenFlowSwitchNetDevice as if it were a
   * real OpenFlow switch.
   *
  ...
  /**
   * \brief Get openflow client (application running on this device) pointer
   */
  Ptr<OpenflowClient>GetOpenflowClient(void);

  /**
   * \param app   Openflow client application to run on this device
   * \brief Assign an openflow client application to this net device
   */
  void SetOpenflowClient(Ptr<OpenflowClient>app);
   ...
   * \return 0 if everything's ok, otherwise an error number.
   */
  int ForwardControlInput(const void *msg,
                          size_t      length);
   ...
   */
  int GetSwitchPortIndex(const ofi::Port& p);
...
private:
   ...
   * \param packet_uid Packet UID; used to fetch the packet and its metadata.
   * \param in_port The index of the port the Packet was initially received on.
   * This port doesn't forward when flooding.
   * \param flood If true, don't send out on the ports with flooding disabled.
   * \return 0 if everything's ok, otherwise an error number.
   */
  int OutputAll(uint32_t packet_uid,
                int      in_port,
                bool     flood);
   ...
   * \param packet_uid Packet UID; used to fetch the packet and its metadata.
   */
  void OutputPacket(uint32_t packet_uid,
                    int      out_port);
   ...
   * generically
   * when we may or may not know the specific port we're outputting on. There
   * are many
   * pre-set types of port options besides a Port that's hooked to our
   * OpenFlowSwitchNetDevice.
   * For example, it could be outputting as a flood, or seeking to output to the
   * controller.
   *
   * \param packet_uid Packet UID; used to fetch the packet and its metadata.
   * \param in_port The index of the port the Packet was initially received on.
   ...
   * to forward.
   */
  void OutputPort(uint32_t packet_uid,
                  int      in_port,
                  int      out_port,
                  bool     ignore_no_fwd);
   ...
   * sent; a value of 0 indicates the entire packet should be sent.
   * \param reason Why the packet is being sent.
   */
  void OutputControl(uint32_t packet_uid,
                     int      in_port,
                     size_t   max_len,
                     int      reason);
   ...
   */
  void SendErrorMsg(uint16_t    type,
                    uint16_t    code,
                    const void *data,
                    size_t      len);
   ...
   * destination,
   * setting the IP source address and destination, setting the TCP/UDP source
   * address
   ...
   */
  void SendFeaturesReply(uint32_t xid);
   ...
   */
  void SendFlowExpired(sw_flow                     *flow,
   ...
   */
  void SendPortStatus(ofi::Port p,
                      uint8_t   status);
  ...
  //    void SendVPortTableFeatures ();   Invalid packet in openflow 1.0
   ...
   * match.
   * If it doesn't match, it forwards the packet to the registered controller,
   * if the flag is set.
   *
   * \param packet_uid Packet UID; used to fetch the packet and its metadata.
   ...
   * isn't matched.
   */
  void RunThroughFlowTable(uint32_t packet_uid,
                           int      port,
   ...
   */
  void* MakeOpenflowReply(size_t   openflow_len,
                          uint8_t  type,
                          ofpbuf **bufferp);
  ...
  ofi::SwitchPacketMetadata& GetPacketBuffer();

  void FreePacketBuffer(uint32_t cookie);

  void SweepExpiredFlows();
  ...
  Mac48Address m_myMacAddress;  ///< Address of this device.
  Ptr<Node>    m_node;          ///< Node this device is installed on.
  Ptr<BridgeChannel> m_channel; ///< Collection of port channels into the
                                // Switch Channel.
  ...
  unsigned m_packetDataLimit;

  Ports_t m_ports;                   ///< Switch's ports

  Ptr<OpenflowClient> m_application; ///< Openflow client application running on
                                     // this device
  ...
  uint16_t m_missSendLen;            ///< Flow Table Miss Send Length;
