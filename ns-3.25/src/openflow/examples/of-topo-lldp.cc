...
NS_LOG_COMPONENT_DEFINE("OfTopoLLDP");

bool verbose                   = true;
bool use_drop                  = false;
static const uint SIM_DURATION = 60;
...
main(int argc, char *argv[])
{
  ...
  if (verbose)
  {
    LogComponentEnable("OfTopoLLDP",              LOG_LEVEL_ALL);
    ...
    LogComponentEnable("LldpHandler",             LOG_LEVEL_ALL);
  }
  ...
  ofApps.Stop(Seconds(5.0));

  NS_LOG_INFO("Configure Tracing.");

  csma.EnablePcap("ctrl", controllers);
  csma.EnablePcap("sw3",  ofSwitches);
